...
if db_server? && node[:ec2][:public_hostname] == node[:dr_replication][node[:environment][:framework_env]][:master][:public_hostname]
...
if db_server? && node[:ec2][:public_hostname] == node[:dr_replication][node[:environment][:framework_env]][:initiate][:public_hostname]
  Chef::Log.info "Configuring keys and installing Xtrabackup for replication"
  include_recipe "eydr::keys"
  include_recipe "eydr::#{node['engineyard']['environment']['db_stack_name'].split(/[0-9]/).first}_master_configuration"
end

if db_server? && node[:ec2][:public_hostname] == node[:dr_replication][node[:environment][:framework_env]][:slave][:public_hostname]
