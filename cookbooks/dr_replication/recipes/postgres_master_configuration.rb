...
bash "update-pg-hba-conf" do
  code "echo 'host    replication     postgres        127.0.0.1/32              md5' >> #{node[:datadir]}/pg_hba.conf"
  not_if "grep 'host    replication     postgres        127.0.0.1/32              md5' #{node[:datadir]}/pg_hba.conf"
end
...
bash "update-pg-hba-conf" do
  code "echo 'host    replication     postgres        localhost              md5' >> #{node[:datadir]}/pg_hba.conf"
  not_if "grep 'host    replication     postgres        localhost              md5' #{node[:datadir]}/pg_hba.conf"
end
...
bash "reload-postgres-config" do
  code "su - postgres -c 'pg_ctl reload -D #{node[:datadir]}'"
end
